...
for i in etcd_hosts; do
    newtotal=$(curl -m 30 http://$i:12379/v2/members | jq '.[] | length')
    if [ -n $lasttotal ]; then
...
crnodes=$(cockroach node ls --insecure --host $first_host)
crcount=$(echo $crnodes | cut -d\  -f1)
if [ $((crcount)) -eq $((lasttotal)) ]; then
